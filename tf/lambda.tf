...
resource "aws_lambda_function" "lambda_logic" {
  ...
  environment {
    variables = {
      ...
      STATE_MACHINE_ARN       = aws_ssm_parameter.statemachine_arn.value
    }
  }
}

resource "aws_lambda_function" "lambda_targets" {
  filename         = var.deploy_type == "local" ? "targets.zip" : "dummy.zip"
  function_name    = "game-demo-targets"
  role             = aws_iam_role.iam_for_lambda.arn
  handler          = "index.handler"
  timeout          = 10
  source_code_hash = filebase64sha256(var.deploy_type == "local" ? "targets.zip" : "dummy.zip")
  runtime          = "nodejs18.x"
  memory_size      = 1024
  publish          = true
...
resource "aws_lambda_alias" "game_demo_targets_alias_arn" {
  name             = "prd"
  function_name    = aws_lambda_function.lambda_targets.function_name
  function_version = aws_lambda_function.lambda_targets.version
  lifecycle {
    ignore_changes = all
  }
}
