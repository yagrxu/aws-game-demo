...
console.log(targets);
const sqs = initSqs();
const ddb = dynamodb.initDynamodbClient(AWS, defaultRegion);
...
function filterHit(targets, shootInfo) {
  ...
  if (possibleHit.length > 0) {
    pointToRemove = getNearestPoint(shootInfo.origin.x, shootInfo.origin.y, targets);
...
function canHitTarget(originX, originY, angle, targetX, targetY) {
  ...
  const distance = Math.abs(slope * targetX - targetY + y_intercept) / Math.sqrt(slope * slope + 1);
  return distance <= 2;
}
...
updateRecord = dynamodb.updateRecord;

readRecord = dynamodb.readRecord;
